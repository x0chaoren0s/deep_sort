...
def _nn_cosine_distance(x, y):
    ...
    return distances.min(axis=0)    # 返回所有x中到各y的最小距离
...
class NearestNeighborDistanceMetric(object):
    ...
    def __init__(self, metric, args):
        if metric == "euclidean":
            ...
            self._metric = _nn_cosine_distance # 返回所有x中到各y的最小距离
        else:
            raise ValueError(
    ...
    def distance(self, features, targets):
        ...
        for i, target in enumerate(targets):
            cost_matrix[i, :] = self._metric(self.samples[target], features) # 返回所有x=self.samples[target]中到各y=features的最小距离
        return cost_matrix
